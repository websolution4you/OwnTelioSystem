-- Transactional dry-run of 003a. Always ends with ROLLBACK.
-- It reads only occupied court IDs and leaves no role, schema or function behind.
BEGIN;
...
-- This final metadata result must be false/false after rollback.
SELECT
  EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = 'telio_voice') AS schema_still_exists,
  EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'own_telio_runtime') AS role_still_exists;
